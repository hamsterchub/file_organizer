...
def run_main(input_dir, output_dir, is_recursive = False, num_files_processed = 0, num_folders_removed = 0, num_errors = 0):
    ...
    if not os.path.exists(output_dir):
        print(f"Action - The output directory doesn't exist. Creating \"{output_dir}\"")
    ...
    for filename in os.listdir(input_dir):
        ...
        if file_type_handler == 0:
            main_out = run_main(file_path, output_dir, True, num_files_processed, num_folders_removed, num_errors)
            num_files_processed += main_out[0]
            num_folders_removed += main_out[1]
            num_errors += main_out[2]
            continue
        ...
        # Get full output directory and create if doesn't exist
        create_out_dir_return = util_functions.create_output_dir(output_dir, partial_output_path)
        if (create_out_dir_return == 0):    # There was an error in creating the output directory
            num_errors += 1
            continue
        print(f"Log - No error in creating output directory \"{create_out_dir_return}\"")

        # Check to see if the current file already exists in the appropriate destination folder and attempt to delete it
        bool_find_dupes_out = util_functions.find_dupes(create_out_dir_return, filename)
        if not bool_find_dupes_out: # File was not successfully deleted
            num_errors += 1
            continue
        print(f"Log - No error in deleting \"{file_path}\"")
        
        # Move the file to the full output path subdirectory and increment successful procs by 1
        shutil.move(file_path, create_out_dir_return)
        print(f"Action - Moved: \"{filename}\" to \"{create_out_dir_return}\"\n")
        num_files_processed += 1

    # Checks for empty input directory. Delete the input_dir unless is_recursive is set to FALSE
    if util_functions.is_dir_empty(input_dir):        
        if is_recursive:
            bool_cleanup_output = util_functions.remove_empty_dir(input_dir) # False if cleanup had an error, True otherwise
...
if __name__ == "__main__":
    ...
    input_dir = "E:\Development\Python\Input"
    output_dir = "E:\Development\Python\Output"
    print(f"Log - Input directory: \"{input_dir}\"\nLog - Output directory: \"{output_dir}\"")

    print(f"Log - Entering main\n")
    main_out = run_main(input_dir, output_dir)
